...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        if ((strcmp(argv[i], "-N") == 0) || (strcmp(argv[i], "-Rows") == 0)) {
            ...
            printf("  -Rows (-N) <int>:      exponent num, determines number of "
                   "rows 2^num (default: 2^12 = 4096)\n");
            printf("  -Columns (-M) <int>:   exponent num, determines number of "
                   "columns 2^num (default: 2^10 = 1024)\n");
            printf("  -Size (-S) <int>:      exponent num, determines total "
                   "matrix size 2^num (default: 2^22 = 4096*1024 )\n");
...

    {
        ...
        for (int repeat = 0; repeat < nrepeat; repeat++) {
            ...
            // Output result.
            if (repeat == (nrepeat - 1)) { printf("  Computed result for %d x %d is %lf\n", N, M, result); }

            const double solution = (double)N * (double)M;

            if (result != solution) { printf("  Error: result( %lf ) != solution( %lf )\n", result, solution); }
        }
