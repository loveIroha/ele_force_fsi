...
using UserSolidSolver = dolfin::ValveProblemSolver;
...
double current_pressure(double t) {
    ...
    // calculation
    int n = t / dt;
    CHECK_F(n <= pressure_num - 2, "out of time range.");
    double k = (t - n) / dt;
    return (pressure_list[n + 1] - pressure_list[n]) * k;
}
...
int time_immersed_boundary_method2() {
    ...
    // Define solvers.
    LOG_F(INFO, "Creating fluid sover and solid solver.");
    auto fluid_solver
