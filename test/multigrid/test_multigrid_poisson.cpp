...
int test_poisson_1d(int length) {
    // Define the mesh.
    ...
    PoissonProblem1D<VectorType, BCVectorType> cpu_mgb(dim, dh);
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        cpu_mgb.compute_residuals(x, b, r, dim, dh);
        LOG_F(INFO, "the residual    of %ld th iteration :  %e!", i, r.inner(r) / r.size());

        r.axpy(-1.0, x, x_old);
        LOG_F(INFO, "the convergence of %ld th iteration :  %e!", i, r.inner(r) / r.size());

        r.axpy(-1.0, x, x_exact);
        LOG_F(INFO, "the error       of %ld th iteration :  %e!", i, r.inner(r) / r.size());
    }
    return true;
}
...
bool test_poisson_2d(int size) {
    ...
    // Define the multigrid solver.
    PoissonProblem2D<StdVector<double, double>, BCVectorType> cpu_mgb(dim, dh);
...
bool test_poisson_3d() {
    ...
    // Define the multigrid solver.
    PoissonProblem3D<StdVector<double, double3>, BCVectorType> cpu_mgb(dim, dh);
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        cpu_mgb.compute_residuals(x, b, r, boundary_type, dim, dh);
        LOG_F(INFO, "after %ld th iteration, the residual is    :  %e!", i, r.inner(r) / r.size());

        r.axpy(-1.0, x, x_old);
        LOG_F(INFO, "after %ld th iteration, the convergence is :  %e!", i, r.inner(r) / r.size());

        r.axpy(-1.0, x, x_exact);
        LOG_F(INFO, "after %ld th iteration, the error is       :  %e!", i, r.inner(r) / r.size());
    }
    return true;
}
