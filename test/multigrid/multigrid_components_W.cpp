...
// std::array<int, 6> all_boundary_type = { NEUMANN,  NEUMANN,  NEUMANN,
// NEUMANN,NEUMANN,  NEUMANN};
std::array<int, 6> all_boundary_type = {DIRICHLET, DIRICHLET, DIRICHLET, DIRICHLET, DIRICHLET, DIRICHLET};
int                max_iters         = 20000;
double             tolerance         = 1e-6;

int main_w(int3 N, int Nt, double3 L, double T, double rho, double mu_f) {
    double t  = 0.0;
    double dx = L.x / N.x;
    double dy = L.y / N.y;
    double dz = L.z / N.z;
    ...
    auto xbt     = algebra::create_multi_array<3, int>({N.x + 2, N.y + 2, N.z + 3});
    auto xn      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto xb      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto xbv     = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto f3      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto exact   = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto eh      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto xh      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto rh      = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto x_prime = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});
    auto r_prime = algebra::create_multi_array<3, double>({N.x + 2, N.y + 2, N.z + 3});

    ns_demo->get_boundary_type_w(xbt, all_boundary_type);
    ns_demo->get_array_w(xn, t);

    auto kokkos_solver
        = mykokkos::VelocityWKokkos3D(N.x, N.y, N.z, Nt, L.x, L.y, L.z, T, mu_f, rho, tolerance, max_iters);
    ...
    for (int i = 1; i <= Nt; i++) {
        ...
        // 获取边界条件和右端项
        ns_demo->get_array_f3(f3, t);
        ns_demo->make_tentitive_wb(xb, xn, f3);
        ns_demo->get_boundary_values_w(xbv, xbt, t);
        // 求解
