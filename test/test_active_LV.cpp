...
using UserFluidSolver = maorope::ProjectionSchemeGPU<PressureTypegpu, VelocityTypegpu>;
...
int time_immersed_boundary_method2() {
    ...
    *xn_new = *xn;
    ...
    while (ibm_problem->_t < 10) {
        ...
        if (time > 0.8)
            ibm_problem->_solid_solver->calculate_T(ibm_problem->_t, ibm_problem->_dt);
        else
