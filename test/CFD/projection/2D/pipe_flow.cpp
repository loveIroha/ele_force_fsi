...
int test_lid_driven(int Nt, double T, double h, int Nx, int Ny) {
    // 输入输出
    VTIWriter velocity_writer{"pipe_flow/velocity/data.pvd"};
    ...
    for (size_t j = 1; j < _Ny + 1; j++) {
        double y              = h * j - 0.5 * h;
        stokes_flow.ubv[0][j] = 1.5 * (4.0 * y * (0.41 - y)) / (0.41 * 0.41);
    }
    ...
    for (size_t step = 1; step < 1000000000; step++) {
        ...
        velocity_writer.write(domain_mesh.get_dim(), make_double2(domain_mesh.origin.x, domain_mesh.origin.y),
                              make_double2(domain_mesh._dx, domain_mesh._dy), flatten(cell_center_velocity), t);

        for (size_t j = 0; j < _Ny + 2; j = j + 1)
            LOG_F(INFO, "Velocity at point %f %f is %f %f", (_Nx / 2) * stokes_flow._dx, j * stokes_flow._dy,
                  cell_center_velocity[_Nx / 2][j].x, cell_center_velocity[_Nx / 2][j].y);
