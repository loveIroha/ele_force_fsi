...
#include <PhysicsSolver/SolidSolver/FlowpushCube2D/FlowpushCube2D.h>
...
class PipeFlow {
  public:
...

    {
        ...
        for (size_t j = 1; j < mesh._Ny + 1; j++) {
            double  y             = mesh._dy * j - 0.5 * mesh._dy;
            double2 result        = left_boundary({0.0, y});
            stokes_flow.ubv[0][j] = result.x;
        }
...
class ImmersedBoundaryMethod : public algebra::NonlinearProblem {
  public:
    ...
    PipeFlow                                pipe_flow;
    std::shared_ptr<dolfin::FlowpushCube2D> flow_past_cylinder;
    ...
    ImmersedBoundaryMethod(BasicMesh<DEGREE, DIM> _solid_mesh, BackgroundMesh2D<DIM> _domain_mesh, PipeFlow ns_solver,
                           std::shared_ptr<dolfin::FlowpushCube2D> solid_solver)
...
void solve_one_step(ImmersedBoundaryMethod& ibm) {
    ...
    // // NOTE: X_{n-1} = X_{n} 赋值位移
    // ibm.flow_past_cylinder->X_nm1->vector()->set_local(ibm._solid_displacement_2);
...
void test_1_2(int Nt, int N_s, int N_bg, double T, double eta = 1e4, double beta = 1e4) {
    beta = eta;

    // 算例的目标文件夹
    std::ostringstream path;
    path << "demo_FlowpushCube_2D_explicit_" << N_s << "_" << N_bg << "_" << Nt << "_" << T << "/";
    ...
    auto                   solid_mesh_dolfin = std::make_shared<dolfin::Mesh>(dolfin::RectangleMesh::create(
        {
            dolfin::Point(0.1, 0.1),
            dolfin::Point(0.125, 0.3),
        },
        {2, 16}, dolfin::CellType::Type::triangle));
    ...
    // 固体求解器
    auto flow_past_cylinder = std::make_shared<dolfin::FlowpushCube2D>(solid_mesh_dolfin, eta, beta, dt, path.str());

    // 流体求解器
    PipeFlow pipe_flow(domain_mesh, path.str() + "fluid/velocity/data.pvd", path.str() + "fluid/pressure/data.pvd",
...
int main(int argc, char* argv[]) {
    ...
    std::ostringstream path;
    path << "demo_FlowpushCube_2D_explicit_" << N_s << "_" << N_bg << "_" << Nt << "_" << T << "_" << eta << "/";
