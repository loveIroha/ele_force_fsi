...
class PipeFlow {
  public:
    ...
    PipeFlow(BackgroundMesh2D<2> mesh, std::string output_file = "fluid/data.pvd")
...
class ImmersedBoundaryMethod : public algebra::NonlinearProblem {
  public:
    ...
    PipeFlow                                   _ns_solver;
    std::shared_ptr<dolfin::BarBehindCylinder> _solid_solver;
    ...
    void calculate_solid_velocity(const std::vector<double>& X, std::vector<double>& U) {
        ...
        _ns_solver.set_boundary_values();
...
void solve_one_step(ImmersedBoundaryMethod& ibm) {
    ...
    // 更新固体求解器中的速度
    ibm._solid_solver->Velocity->vector()->set_local(ibm._solid_velocities_2);
    ...
    if (count % 100 == 1) {
        ...
        // 存储固体力和固体位移
        ibm._solid_solver->record<double2, double>(ibm._solid_forces, ibm._solid_displacement, ibm.t);
        ibm.eulerian_area();
...
void test_1_2(int Nt, int N_s, int N_bg, double T, double kappa, double eta, double c1_s, double G_s, double nv,
              double mu_f, double rho_f, double rho_s, std::map<std::string, std::string>& paths) {
    // 参数
    double Lx = 2.46;
    double Ly = 0.41;
    int    Nx = 6 * N_bg;
    ...
    auto materal_types = std::make_shared<dolfin::MeshFunction<std::size_t>>(solid_mesh_dolfin, domains_file);
    auto boundaries    = std::make_shared<dolfin::MeshFunction<std::size_t>>(solid_mesh_dolfin, boundaries_file);
    ...
    // 固体求解器
    auto solid_solver = std::make_shared<dolfin::BarBehindCylinder>(solid_mesh_dolfin, materal_types, boundaries, kappa,
                                                                    eta, c1_s, G_s, nv, output_file);

    // 流体求解器
    PipeFlow fluid_solver(domain_mesh, output_file + "fluid/data.pvd");
...
bool help(int argc, char* argv[]) {
    if (parameters::get_arg(argv, argv + argc, "--help")) {
        printf("带尾圆柱绕流程序。使用方法: \n");
        printf("    程序名\t\t\t\t-参数名\t参数值\n");
        printf("    ./explicit_ibm_bar_behind_cylinder\t-Nt\t10000\n");
        printf("\n");
        printf("可选参数:\n");
...
int main(int argc, char* argv[]) {
    ...
    int    Ns = parameters::get_argval<int>(argv, argv + argc, "-Ns", 20);
    int    Nb = parameters::get_argval<int>(argv, argv + argc, "-Nb", 64);
    int    Nt = parameters::get_argval<int>(argv, argv + argc, "-Nt", 1000000);
    double T  = parameters::get_argval<double>(argv, argv + argc, "-T", 50);

    double kappa = parameters::get_argval<double>(argv, argv + argc, "-kappa", 1.2e9);
    double eta   = parameters::get_argval<double>(argv, argv + argc, "-eta", 0.0);

    double Gs    = parameters::get_argval<double>(argv, argv + argc, "-Gs", 2.0e6);
    double nv    = parameters::get_argval<double>(argv, argv + argc, "-nv", 0.49);
    double c1_s  = parameters::get_argval<double>(argv, argv + argc, "-c1_s", 5000.0);
    double rho_s = parameters::get_argval<double>(argv, argv + argc, "-rho_s", 1000.0);

    double mu_f  = parameters::get_argval<double>(argv, argv + argc, "-mu_f", 1.0);
    double rho_f = parameters::get_argval<double>(argv, argv + argc, "-rho_f", 1000.0);

    std::map<std::string, std::string> paths;

    // 插入键值对
    std::ostringstream path;
    path << "demo_BarBehindCylinder_2D_explicit_" << Ns << "_" << Nb << "_" << Nt << "_" << T << "_" << kappa << "_"
         << eta << "_" << c1_s << "_" << Gs << "_" << nv << "_" << rho_s << "_" << rho_f << "_" << mu_f << "/";
    ...
    auto boundaries_file = geometry_path("temp/bar_behind_cylinder_");
    boundaries_file += std::to_string(Ns);
    boundaries_file += "_boundaries.xml";
    ...
    test_1_2(Nt, Ns, Nb, T, kappa, eta, c1_s, Gs, nv, mu_f, rho_f, rho_s, paths);

    success_notification_with_163_email(path.str());
}
