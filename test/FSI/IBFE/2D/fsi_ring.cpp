...
void test_1_2(int Nt, int N_s, int N_bg, double T, std::string path) {
    ...
    solid_solver->set_initial_configuration(ibm._solid_displacement_2);
    ibm._solid_displacement = algebra::ripple<double2, double>(ibm._solid_displacement_2);
    // 开始循环
...
int main(int argc, char* argv[]) {
    ...
    {
        char buffer[100];
        snprintf(buffer, sizeof(buffer), "demo_static_ring_2D_plicit_%03d_%04d_%05d_%.0f/", N_s, N_bg, Nt, T);
        path = buffer;
        // 日志文件
