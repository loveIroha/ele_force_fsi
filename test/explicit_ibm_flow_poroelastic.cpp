...
void test_1_2(int Nt, int N_s, int N_bg, double T, double eta = 1e4, double beta = 1e4) {
    ...
    path << "demo_FlowPastCylinder_2D_explicit_" << N_s << "_" << N_bg << "_" << Nt << "_" << T << "/";
    ...
    // 固体求解器
    auto flow_past_cylinder = std::make_shared<dolfin::FlowPastCylinder>(solid_mesh_dolfin, eta, beta, dt, path.str());
...
int main(int argc, char* argv[]) {
    ...
    std::ostringstream path;
    path << "demo_FlowPastCylinder_2D_explicit_" << N_s << "_" << N_bg << "_" << Nt << "_" << T << "_" << eta << "/";
