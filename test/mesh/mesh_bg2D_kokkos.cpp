...
void test_interpolation_raw(std::shared_ptr<dolfin::Mesh> solid_mesh_dolfin, BackgroundMesh2D<DIM> domain_mesh) {
    ...
    interpolate_u(result, eulerian_u.data(), qr, {domain_mesh._dx, domain_mesh._dy}, dim_u, 2);
    printf("%.20f %.20f\n", result[0], result[1]);
    ...
    result[0] = 0.0;
    result[1] = 0.0;
    interpolate_v(result, eulerian_v.data(), qr, {domain_mesh._dx, domain_mesh._dy}, dim_v, 2);
    printf("%.20f %.20f\n", result[0], result[1]);
}

void test_distribute_raw(std::shared_ptr<dolfin::Mesh> solid_mesh_dolfin, BackgroundMesh2D<DIM> domain_mesh) {
    // 给eulerian_u和eulerian_v赋值
    std::vector<double> eulerian_u(domain_mesh.get_size_u());
    std::vector<double> eulerian_v(domain_mesh.get_size_v());
    auto                dim_u = domain_mesh.get_dim_u();
    ...
    // use cpu
    distribute_u(input, eulerian_u.data(), 2, dim_u, domain_mesh._dx, domain_mesh._dy, qr);
    std::string filename = "u_cpu.txt";
    IO::write_vector_2D(algebra::ripple(eulerian_u, dim_u.x), filename);
    distribute_v(input, eulerian_v.data(), 2, dim_v, domain_mesh._dx, domain_mesh._dy, qr);
    filename = "v_cpu.txt";
    IO::write_vector_2D(algebra::ripple(eulerian_v, dim_v.x), filename);

    // use kokkos
    std::fill(eulerian_u.begin(), eulerian_u.end(), 0);
    distribute_u(eulerian_u.data(), input, qr, {domain_mesh._dx, domain_mesh._dy}, dim_u, 2);

    filename = "u_kokkos.txt";
    IO::write_vector_2D(algebra::ripple(eulerian_u, dim_u.x), filename);

    distribute_v(eulerian_v.data(), input, qr, {domain_mesh._dx, domain_mesh._dy}, dim_v, 2);

    filename = "v_kokkos.txt";
    IO::write_vector_2D(algebra::ripple(eulerian_v, dim_v.x), filename);
}
...
int test_interpolation(std::shared_ptr<dolfin::Mesh> solid_mesh_dolfin, BackgroundMesh2D<DIM> domain_mesh) {
    ...
    for (size_t logf_i = 0; logf_i < data1.size(); logf_i++) {
        ...
        // %.16e %.16e.", logf_i, data2[logf_i].x, data2[logf_i].y); LOG_F(INFO,
        // "error                  : %zu %.20e %.20e.", logf_i, data2[logf_i].y
        // - data1[logf_i].x, data2[logf_i].x - data1[logf_i].y);
    }
    ...
    //     x[0]); LOG_F(INFO, "cooridnates: %.20f %.20f", cooridnates[i * 2 +
    //     0], cooridnates[i * 2 + 1]); LOG_F(INFO, "values     : %.20f %.20f",
    //     values[0], values[1]);
    // }
...
void test_2_1() {
    ...
    test_interpolation_raw(solid_mesh_dolfin, domain_mesh);
    test_distribute_raw(solid_mesh_dolfin, domain_mesh);
