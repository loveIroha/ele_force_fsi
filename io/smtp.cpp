// 参考自 https://blog.csdn.net/nihaoqlw/article/details/88692197
...
char* CSmtp::base64Encode(const char* origSigned, unsigned origLength) {
    const unsigned char* orig = (const unsigned char*)origSigned; // in case any input bytes have the MSB set
    if (orig == NULL) return NULL;

    const unsigned numOrig24BitValues = origLength / 3;
    bool           havePadding        = origLength > numOrig24BitValues * 3;
    bool           havePadding2       = origLength == numOrig24BitValues * 3 + 2;
    const unsigned numResultBytes     = 4 * (numOrig24BitValues + havePadding);
...
CSmtp::CSmtp(int port, string srvDomain, string userName, string password, string targetEmail, string emailTitle,
             string content) {
...
int CSmtp::Login() {
    ...
    if (false == Send(sendBuff) || false == Recv()) return 1;
    ...
    if (NULL != strstr(buff, "550")) return 2;

    if (NULL != strstr(buff, "535")) return 3;
    return 0;
}
...
bool CSmtp::SendEmailHead() {
    ...
    if (false == Send(sendBuff)) return false;
...
int CSmtp::SendEmail_Ex() {
    if (false == CreateConn()) return 1;
    int err = Login();
    if (err != 0) return err;
    if (false == SendEmailHead()) return 1;
    if (false == SendTextBody()) return 1;
    if (false == SendEnd()) return 1;
    return 0;
}

int smtp::email_notification(CSmtp& smtp) {
    int err;
    if ((err = smtp.SendEmail_Ex()) != 0) {
        if (err == 1) cout << "错误1: 由于网络不畅通，发送失败!" << endl;
        if (err == 2) cout << "错误2: 用户名错误,请核对!" << endl;
        if (err == 3) cout << "错误3: 用户密码错误，请核对!" << endl;
        if (err == 4) cout << "错误4: 请检查附件目录是否正确，以及文件是否存在!" << endl;
    }
    return err;
}