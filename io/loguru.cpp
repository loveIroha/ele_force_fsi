...
#define ENABLE_VIRTUAL_TERMINAL_PROCESSING 0x0004
    ...
    if (const char* term = getenv("TERM")) {
        return 0 == strcmp(term, "cygwin") || 0 == strcmp(term, "linux") || 0 == strcmp(term, "rxvt-unicode-256color")
               || 0 == strcmp(term, "screen") || 0 == strcmp(term, "screen-256color")
               || 0 == strcmp(term, "screen.xterm-256color") || 0 == strcmp(term, "tmux-256color")
               || 0 == strcmp(term, "xterm") || 0 == strcmp(term, "xterm-256color")
...
void file_reopen(void* user_data) {
    ...
    if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || (st.st_ino != file_abs->st.st_ino)) {
        ...
        if (!file_abs->fp) {
            ...
            VLOG_F(g_internal_verbosity, "Reopening file '" LOGURU_FMT(s) "' due to '" LOGURU_FMT(s) "'",
...
#define getcwd _getcwd
    ...
    if (strlen(s_current_dir) != 0) { VLOG_F(g_internal_verbosity, "Current dir: " LOGURU_FMT(s) "", s_current_dir); }
...
void write_date_time(char* buff, size_t buff_size) {
    ...
    snprintf(buff, buff_size, "%04d%02d%02d_%02d%02d%02d.%03lld", 1900 + time_info.tm_year, 1 + time_info.tm_mon,
             time_info.tm_mday, time_info.tm_hour, time_info.tm_min, time_info.tm_sec, ms_since_epoch % 1000);
}
...
bool add_file(const char* path_in, FileMode mode, Verbosity verbosity) {
    ...
    if (!create_directories(path)) { LOG_F(ERROR, "Failed to create directories to '" LOGURU_FMT(s) "'", path); }
    ...
    VLOG_F(g_internal_verbosity,
           "Logging to '" LOGURU_FMT(s) "', mode: '" LOGURU_FMT(s) "', verbosity: " LOGURU_FMT(d) "", path, mode_str,
           verbosity);
    return true;
}
...
std::string stacktrace_as_stdstring(int skip) {
    ...
    for (int i = num_frames - 1; i >= skip; --i) {
        ...
        if (dladdr(callstack[i], &info) && info.dli_sname) {
            char* demangled = NULL;
            int   status    = -1;
            if (info.dli_sname[0] == '_') { demangled = abi::__cxa_demangle(info.dli_sname, 0, 0, &status); }
...
static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line) {
    ...
    if (g_preamble_date && pos < out_buff_size) {
        int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%04d-%02d-%02d ", 1900 + time_info.tm_year,
                             1 + time_info.tm_mon, time_info.tm_mday);
        if (bytes > 0) { pos += bytes; }
    }
    if (g_preamble_time && pos < out_buff_size) {
        int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%02d:%02d:%02d.%03lld ", time_info.tm_hour,
    ...
    if (g_preamble_file && pos < out_buff_size) {
        ...
        int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%*s:%-5u ", LOGURU_FILENAME_WIDTH,
                             shortened_filename, line);
        if (bytes > 0) { pos += bytes; }
    }
    if (g_preamble_verbose && pos < out_buff_size) {
...
static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal) {
    ...
    if (message.verbosity == Verbosity_FATAL) {
        ...
        if (!st.empty()) { RAW_LOG_F(ERROR, "Stack trace:\n" LOGURU_FMT(s) "", st.c_str()); }

        auto ec = loguru::get_error_context();
        if (!ec.empty()) { RAW_LOG_F(ERROR, "" LOGURU_FMT(s) "", ec.c_str()); }
    }
    ...
    if (verbosity <= g_stderr_verbosity) {
        if (g_colorlogtostderr && s_terminal_has_color) {
            if (verbosity > Verbosity_WARNING) {
                fprintf(stderr, "%s%s%s%s%s%s%s%s\n", terminal_reset(), terminal_dim(), message.preamble,
                        message.indentation,
                        verbosity == Verbosity_INFO ? terminal_reset() : "", // un-dim for info
                        message.prefix, message.message, terminal_reset());
            } else {
                fprintf(stderr, "%s%s%s%s%s%s%s\n", terminal_reset(),
                        verbosity == Verbosity_WARNING ? terminal_yellow() : terminal_red(), message.preamble,
                        message.indentation, message.prefix, message.message, terminal_reset());
            }
        } else {
...
// stack_trace_skip is just if verbosity == FATAL.
void log_to_everywhere(int stack_trace_skip, Verbosity verbosity, const char* file, unsigned line, const char* prefix,
                       const char* buff) {
...
LogScopeRAII::LogScopeRAII(Verbosity verbosity, const char* file, unsigned line, const char* format, ...)
    ...
    if (verbosity <= current_verbosity_cutoff()) {
        ...
        for (auto& p : s_callbacks) {
            if (verbosity <= p.verbosity) { ++p.indentation; }
        }
    } else {
        _file = nullptr;
    }
...
#if LOGURU_USE_FMTLIB
void vlog_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, const char* format,
...
// ----------------------------------------------------------------------------
// 888888 88""Yb 88""Yb  dP"Yb  88""Yb      dP""b8  dP"Yb  88b 88 888888
// 888888 Yb  dP 888888 88__   88__dP 88__dP dP   Yb 88__dP     dP   `" dP Yb
// 88Yb88   88   88__    YbdP    88 88""   88"Yb  88"Yb  Yb   dP 88"Yb      Yb
// Yb   dP 88 Y88   88   88""    dPYb    88 888888 88  Yb 88  Yb  YbodP  88 Yb
...
#if LOGURU_USE_FMTLIB
            auto prefix = textprintf("[ErrorContext] {.{}s}:{:-5u} {:-20s} ", filename(entry->_file),
                                     LOGURU_FILENAME_WIDTH, entry->_line, description.c_str());
#else
            auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ", LOGURU_FILENAME_WIDTH, filename(entry->_file),
...
EcEntryBase::EcEntryBase(const char* file, unsigned line, const char* descr) : _file(file), _line(line), _descr(descr) {
