...
template <class T>
struct _is_default_printable<
    T, std::void_t<std::pair<typename _printer<T>::is_default_print,
...
template <class T, class U>
struct _enable_if_iterable<
    T, U, std::void_t<typename std::iterator_traits<decltype(std::begin(std::declval<const T&>()))>::value_type>> {
    using type = U;
};
...
template <class T, class U>
struct _enable_if_has_print<T, U, std::void_t<decltype(std::declval<const T&>().do_print())>> {
    using type = U;
};

/* template <class T, class U> */
/* struct _enable_if_iterable<T, U, std::void_t<typename
 * std::iterator_traits<typename T::const_iterator>::value_type>>
 * { */
/*     using type = U; */
/* }; */
...
struct _printer<T, typename _enable_if_has_print<T>::type> {
    static void print(const T& t) { t.do_print(); }
};

template <class T>
struct _printer<
    T, typename _enable_if_has_print<
           T, typename _enable_if_iterable<
                  T, typename _enable_if_c_str<T, typename _enable_if_string<T, typename _enable_if_map<T>::not_type>::
                                                      not_type>::not_type>::type>::not_type> {
...
template <class T>
struct _printer<T, typename _enable_if_has_print<
                       T, typename _enable_if_tuple<T, typename _enable_if_iterable<T>::not_type>::type>::not_type> {
    template <std::size_t... Is>
    static void _unrolled_print(const T& t, std::index_sequence<Is...>) {
        std::cout << "{";
        ((_printer<_rmcvref_t<std::tuple_element_t<Is, T>>>::print(std::get<Is>(t)), std::cout << ", "), ...);
        if constexpr (sizeof...(Is) != 0)
            _printer<_rmcvref_t<std::tuple_element_t<sizeof...(Is), T>>>::print(std::get<sizeof...(Is)>(t));
        std::cout << "}";
    }
...
struct _printer<T, typename _enable_if_has_print<T, typename _enable_if_string<T>::type>::not_type> {
    static void print(const T& t) { std::cout << std::quoted(t); }
};
...
struct _printer<std::nullptr_t, void> {
    static void print(const std::nullptr_t&) { std::cout << "nullptr"; }
};

template <>
struct _printer<std::nullopt_t, void> {
    static void print(const std::nullopt_t&) { std::cout << "nullopt"; }
};

template <>
struct _printer<std::monostate, void> {
    static void print(const std::monostate&) { std::cout << "monostate"; }
};
...
struct _printer<T, typename _enable_if_has_print<T, typename _enable_if_variant<T>::type>::not_type> {
    static void print(const T& t) {
        std::visit([](const auto& v) { _printer<_rmcvref_t<decltype(v)>>::print(v); }, t);
    }
};
...
void print(T0 const& t0, const Ts&... ts) {
...
template <class T0, class... Ts>
void printnl(T0 const& t0, const Ts&... ts) {
    _printer<_rmcvref_t<T0>>::print(t0);
    ((std::cout << " ", _printer<_rmcvref_t<Ts>>::print(ts)), ...);
}
...
class print_adaptor {
  ...
  public:
    explicit constexpr print_adaptor(const T& t_) : t(t_) {}

    friend std::ostream& operator<<(std::ostream& os, const print_adaptor&& self) {
        auto oldflags = os.flags();
        os << "[object 0x" << std::hex
           << reinterpret_cast<uintptr_t>(reinterpret_cast<const volatile void*>(std::addressof(self.t))) << ']';
        os.flags(oldflags);
        return os;
    }
};
...
template <class T>
explicit print_adaptor(const T&) -> print_adaptor<T>;
} // namespace _print_details
