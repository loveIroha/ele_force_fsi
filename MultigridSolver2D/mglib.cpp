...
vector2D<double> restrict(const vector2D<double>& fine, int Nx, int Ny) {
    auto coarse = IO::make_vector_2D<double>(Nx + 2, Ny + 2);
    for (int i = 1; i <= Nx; ++i) {
        for (int j = 1; j <= Ny; ++j) {
            coarse[i][j]
                = 0.25
...
vector2D<int> restrict_bc(const vector2D<int>& fine, int Nx, int Ny) {
    vector2D<int> coarse = IO::make_vector_2D<int>(Nx + 2, Ny + 2);
...
vector2D<double> interpolate(const vector2D<double>& coarse, int Nx, int Ny) {
    ...
    for (int i = 0; i <= Nx; ++i) {
        for (int j = 0; j <= Ny; ++j) {
            ...
            fine[2 * i][2 * j]         = bilinear(1.0 / 4.0, 1.0 / 4.0, f);
            fine[2 * i][2 * j + 1]     = bilinear(1.0 / 4.0, 3.0 / 4.0, f);
            fine[2 * i + 1][2 * j]     = bilinear(3.0 / 4.0, 1.0 / 4.0, f);
            fine[2 * i + 1][2 * j + 1] = bilinear(3.0 / 4.0, 3.0 / 4.0, f);
        }
    }
...
std::vector<std::vector<double>> compute_residual(std::vector<std::vector<double>>&       phi,
    ...
    auto residual = IO::make_vector_2D<double>(Nx + 2, Ny + 2);
    for (int i = 1; i <= Nx; ++i) {
...
std::vector<std::vector<double>> generate_u_prime(const std::vector<std::vector<int>>&    boundary_type,
    ...
    double dx  = width / Nx;
    double dy  = height / Ny;
    auto   phi = IO::make_vector_2D<double>(Nx + 2, Ny + 2);

    for (int i = 1; i <= Nx; ++i) {
        if (boundary_type[i][0] == DIRICHLET) { phi[i][0] = 2 * boundary_values[i][0] - phi[i][1]; }
        if (boundary_type[i][0] == NEUMANN) { phi[i][0] = phi[i][1] - dx * boundary_values[i][0]; }
        if (boundary_type[i][Ny + 1] == DIRICHLET) { phi[i][Ny + 1] = 2 * boundary_values[i][Ny + 1] - phi[i][Ny]; }
        if (boundary_type[i][Ny + 1] == NEUMANN) { phi[i][Ny + 1] = phi[i][Ny] + dx * boundary_values[i][Ny + 1]; }
    }
    for (int j = 1; j <= Ny; ++j) {
