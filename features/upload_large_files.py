...
def upload_file_to_release_id(release_id, filename, path = './'):
    ...
    params = {
        "name": filename.split('.')[0] + datetime.datetime.now().strftime('%Y%m%d%H%M%S')+'.'+filename.split('.')[-1],
    }
    with open(path+filename, "rb") as f:
        response = requests.post(url, headers=headers, params=params, data=f)

    print(f"Uploading {filename}", file=log)

    if response.status_code == 201:
        asset_url = response.json()["browser_download_url"]
        print(f"File uploaded successfully to {asset_url}", file=log)
    else:
        print(f"Failed to upload file. Response code: {response.status_code}", file=log)
...
release_id = release['id']
if release:
    print('Latest release:', release['tag_name'], file=log)
else:
    print('Error: unable to retrieve latest release', file=log)


files = ['position000170.vtu']
for file in files:
    upload_file_to_release_id(release_id, file)
