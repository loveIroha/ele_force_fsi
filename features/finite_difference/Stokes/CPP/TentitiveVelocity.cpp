...
int tentitive_velocity_u(const std::vector<std::vector<double>> f,  //(Nx+1)*(Ny+2)
    ...
    tentitive_velocity_u(ub, phi, boundary_type, boundary_values, width, height, T, rho, mu, max_iters);
}

int main() {
    auto ub              = make_vector_2D<double>(Nx + 1, Ny + 2);
    auto phi             = make_vector_2D<double>(Nx + 1, Ny + 2);
    auto boundary_values = make_vector_2D<double>(Nx + 1, Ny + 2);
    auto boundary_type   = make_vector_2D<int>(Nx + 1, Ny + 2);
    ...
    // 设置边界条件
    int boundary_edge_type[4] = {2, 2, 2, 1};
    for (size_t i = 0; i < Nx + 1; i++) {
    ...
    for (size_t j = 1; j < Ny + 1; j++) {
        ...
        boundary_type[Nx][j] = boundary_edge_type[3];
    }
    ...
    // 计算边界的值
    for (size_t i = 0; i < Nx + 1; i++) {
        if (boundary_type[i][0] == DIRICHLET) { boundary_values[i][0] = fun_u(dx * i, 0); }
        if (boundary_type[i][0] == NEUMANN) { boundary_values[i][0] = fun_dudy(dx * i, 0); }
        if (boundary_type[i][Ny] == DIRICHLET) { boundary_values[i][Ny] = fun_u(dx * i, 1); }
        if (boundary_type[i][Ny] == NEUMANN) { boundary_values[i][Ny] = fun_dudy(dx * i, 1); }
    }
    for (size_t j = 1; j < Ny + 1; j++) {
        if (boundary_type[0][j] == DIRICHLET) { boundary_values[0][j] = fun_u(0, dy * j - 0.5 * dy); }
        if (boundary_type[0][j] == NEUMANN) { boundary_values[0][j] = fun_dudx(0, dy * j - 0.5 * dy); }
        if (boundary_type[Nx][j] == DIRICHLET) { boundary_values[Nx][j] = fun_u(1, dy * j - 0.5 * dy); }
        if (boundary_type[Nx][j] == NEUMANN) { boundary_values[Nx][j] = fun_dudx(1, dy * j - 0.5 * dy); }
    }
