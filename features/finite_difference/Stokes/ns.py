...
def test_momentum(dt, Nx, Ny):
    ...
    for i in range(100000000):
        ...
        error_u = np.sqrt(np.sum((u - demo_nv.array_u(t))**2)*demo_nv.dx*demo_nv.dy)
        error_v = np.sqrt(np.sum((v - demo_nv.array_v(t))**2)*demo_nv.dx*demo_nv.dy)

    print("time : ", t)
    print("Error of u", np.sum((u - demo_nv.array_u(t))**2)*demo_nv.dx*demo_nv.dy)
    print("Error of v", np.sum((v - demo_nv.array_v(t))**2)*demo_nv.dx*demo_nv.dy)
    print("Error of u_", np.sum((u_ - demo_nv.array_u(t))**2)*demo_nv.dx*demo_nv.dy)
    print("Error of v_", np.sum((v_ - demo_nv.array_v(t))**2)*demo_nv.dx*demo_nv.dy)
    # from rhs import test_divergence_free
    # test_divergence_free(u_, v_, Nx=demo_nv.Nx, Ny=demo_nv.Ny,
    #                         dx=demo_nv.dx, dy=demo_nv.dy, dt=demo_nv.dt, rho=demo_nv.rho)

    # print("before")
    # test_divergence_free(u, v, Nx=demo_nv.Nx, Ny=demo_nv.Ny,
    #                         dx=demo_nv.dx, dy=demo_nv.dy, dt=demo_nv.dt, rho=demo_nv.rho)
    return error_u
