...
def make_example(u):
    ...
    lambda_grad_u =  sym.lambdify(x, sym.diff(u, x, 1), 'numpy')
    lambda_laplace_u = sym.lambdify(x, sym.diff(u, x, 2), 'numpy')
    return lambda_u, lambda_grad_u, lambda_laplace_u
...
def fun(N=16, max_iters=100,u_1=u_1):
    ...
    for iter in range(max_iters):
        ...
        if boundary_type[0] == DIRICHLET:
            phi[0]=2*u(0.0)-phi[1]
        elif boundary_type[0] == NEUMANN:
            # (phi[1]-phi[0])/dx=du(0.0)
            phi[0]=phi[1]-dx*du(0.0)
        if boundary_type[N+1] == DIRICHLET:
            phi[N+1]=2*u(1.0)-phi[N]
        elif boundary_type[N+1] == NEUMANN:
