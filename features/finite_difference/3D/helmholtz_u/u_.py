    ...
    for i in range(0, Nx+1):
        for k in range(0, Nz):
            ...
            if boundary_type[i, 0, k + 1] == NEUMANN:
                u[i, 0, k + 1] = u[i, 1, k + 1] - dy * boundary_values[i, 0, k + 1]
            if boundary_type[i, Ny + 1, k + 1] == DIRICHLET:
                u[i, Ny + 1, k + 1] = 2 * boundary_values[i,Ny + 1, k + 1] - u[i, Ny, k + 1]
            if boundary_type[i, Ny + 1, k + 1] == NEUMANN:
                u[i, Ny + 1, k + 1] = u[i, Ny, k + 1] + dy * boundary_values[i, Ny + 1, k + 1]
