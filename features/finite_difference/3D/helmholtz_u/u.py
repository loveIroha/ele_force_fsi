    ...
    for i in range(0, Nx+1):
        for k in range(0, Nz):
            if boundary_type[i, 0, k + 1] == DIRICHLET:
                data[i, 0, k + 1] = 2 * \
                    boundary_values[i, 0, k + 1] - data[i, 1, k + 1]
            if boundary_type[i, 0, k + 1] == NEUMANN:
                data[i, 0, k + 1] = data[i, 1, k + 1] - \
                    dy * boundary_values[i, 0, k + 1]
            if boundary_type[i, Ny + 1, k + 1] == DIRICHLET:
                data[i, Ny + 1, k + 1] = 2 * boundary_values[i,
                                                             Ny + 1, k + 1] - data[i, Ny, k + 1]
            if boundary_type[i, Ny + 1, k + 1] == NEUMANN:
                data[i, Ny + 1, k + 1] = data[i, Ny, k + 1] + \
    ...
    for i in range(0, Nx+1):
        for j in range(0, Ny):
            if boundary_type[i, j + 1, 0] == DIRICHLET:
                data[i, j + 1, 0] = 2 * \
                    boundary_values[i, j + 1, 0] - data[i, j + 1, 1]
            if boundary_type[i, j + 1, 0] == NEUMANN:
                data[i, j + 1, 0] = data[i, j + 1, 1] - \
                    dz * boundary_values[i, j + 1, 0]
            if boundary_type[i, j + 1, Nz + 1] == DIRICHLET:
                data[i, j + 1, Nz + 1] = 2 * boundary_values[i,
                                                             j + 1, Nz + 1] - data[i, j + 1, Nz]
            if boundary_type[i, j + 1, Nz + 1] == NEUMANN:
                data[i, j + 1, Nz + 1] = data[i, j + 1, Nz] + \
                    dz * boundary_values[i, j + 1, Nz + 1]

    for j in range(0, Ny):
        for k in range(0, Nz):
            if boundary_type[0, j + 1, k + 1] == DIRICHLET:
                data[0, j + 1, k + 1] = boundary_values[0, j+1, k+1]
            if boundary_type[0, j + 1, k + 1] == NEUMANN:
                # u_ghost 为 u[-1, j+1, k+1]
                u_ghost = data[1, j + 1, k + 1]-2.0 * \
                    dx*boundary_values[0, j + 1, k + 1]
                b1 = (u_ghost + data[1,     j + 1, k + 1]) / dx/dx
                b2 = (data[0,     j,     k + 1] +
                      data[0,     j + 2, k + 1]) / dy/dy
                b3 = (data[0,     j + 1, k] +
                      data[0,     j + 1, k + 2]) / dz/dz
                b = mu*(b1+b2+b3)
                a = rho/dt+2*mu*(1/dx/dx+1/dy/dy+1/dz/dz)
                data[0, j+1, k+1] = (bu[0, j+1, k+1]+b)/a
            if boundary_type[Nx, j + 1, k + 1] == DIRICHLET:
                data[Nx, j + 1, k + 1] = boundary_values[Nx, j+1, k+1]
            if boundary_type[Nx, j + 1, k + 1] == NEUMANN:
                u_ghost = data[Nx-1, j + 1, k + 1]+2.0 * \
                    dx*boundary_values[Nx, j + 1, k + 1]
                b1 = (data[Nx - 1, j + 1, k + 1] + u_ghost) / dx/dx
                b2 = (data[Nx,     j,     k + 1] +
                      data[Nx,     j + 2, k + 1]) / dy/dy
                b3 = (data[Nx,     j + 1, k] +
                      data[Nx,     j + 1, k + 2]) / dz/dz
                b = mu*(b1+b2+b3)
                a = rho/dt+2*mu*(1/dx/dx+1/dy/dy+1/dz/dz)
                data[Nx, j+1, k+1] = (bu[Nx, j+1, k+1]+b)/a
...
def fun(N, Nt):
    ...
    for i in range(1, Nt+1):
        ...
        for iter in range(max_iters):
            ...
            print(f"e_norm2: {np.sqrt(np.sum(e**2)*dx*dy*dz):.20e}")
