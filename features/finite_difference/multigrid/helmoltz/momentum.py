...
def test_momentum(dt, Nx, Ny):
    ...
    all_boundary = (DIRICHLET, DIRICHLET, DIRICHLET, DIRICHLET)
    ...
    for i in range(100000000):
        ...
        ub = make_tentitive_u_b(un, f1n, Nx, Ny, dt, rho)
        vb = make_tentitive_v_b(vn, f2n, Nx, Ny, dt, rho)

        u_, residual_u_ = solve_helmholtz_u(
            un, ub, mu, rho, width, height, all_boundary, Nx, Ny, dt, max_iters=10000)
        v_, residual_v_ = solve_helmholtz_v(
            vn, vb, mu, rho, width, height, all_boundary, Nx, Ny, dt, max_iters=10000)
    ...
    error_u = np.sqrt(np.sum((u_ - demo_nv.array_u(t))**2)
                      * demo_nv.dx*demo_nv.dy)
    error_v = np.sqrt(np.sum((v_ - demo_nv.array_v(t))**2)
                      * demo_nv.dx*demo_nv.dy)
...
RR_list = []
dt = 0.1
demo_nv = NavierStokesDemo(1.0, 1.0)
for i in range(4, 7):
    Nx = 8+4*i
    Ny = 8+4*i
    E_list = []
