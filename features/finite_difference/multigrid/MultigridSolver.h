...
class MultigridSolver2D {
  public:
    ...
    MultigridSolver2D(int Nx, int Ny, double width, double height, vector2D<int> boundary_type,
                      ...
                      int num_presmooth = 2, int num_aftersmooth = 2, int num_exactsmooth = 100, int max_iteration = 50,
                      int N_coarsest = 3)
        ...
        for (const auto& info : level_info) {
            ...
            multi_x.push_back(make_vector_2D<double>(info.Nx + 2, info.Ny + 2));
            multi_e.push_back(make_vector_2D<double>(info.Nx + 2, info.Ny + 2));
            multi_r.push_back(make_vector_2D<double>(info.Nx + 2, info.Ny + 2));
            multi_b.push_back(make_vector_2D<double>(info.Nx + 2, info.Ny + 2));
        }

        // 计算每一层网格的边界类型
        multi_bc.push_back(boundary_type);
        for (size_t i = 1; i < _num_levels; i++) {
    ...
    std::vector<std::vector<double>> vcycle(std::vector<std::vector<double>>&       x,
        ...
        for (size_t i = 0; i < _max_iteration; i++) {
            xn        = iterate(x, b, 0);
