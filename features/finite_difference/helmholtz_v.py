...
def solve_helmholtz(Nx, Ny, max_iters, raw_u):
    ...
    for i in range(Nx+2):
        for j in range(Ny+1):
            u_exact[i][j] = u(dx*i-0.5*dx, dy*j)
            b[i][j] = f(dx*i-0.5*dx, dy*j)
    ...
    for i in range(1, Nx+1):
        ...
        if boundary_type[i][Ny] == DIRICHLET:
            boundary_values[i][Ny] = u(dx*i-0.5*dx, 1)
        if boundary_type[i][Ny] == NEUMANN:
            boundary_values[i][Ny] = du[1](dx*i-0.5*dx, 1)

    # 左
    # 右
    for j in range(0, Ny+1):
        if boundary_type[0][j] == DIRICHLET:
            boundary_values[0][j] = u(0, dy*j)
        if boundary_type[0][j] == NEUMANN:
            boundary_values[0][j] = du[0](0, dy*j)
        if boundary_type[Nx+1][j] == DIRICHLET:
            boundary_values[Nx+1][j] = u(1, dy*j)
        if boundary_type[Nx+1][j] == NEUMANN:
            boundary_values[Nx+1][j] = du[0](1, dy*j)
    ...
    for iter in range(max_iters):
        ...
        for i in range(1, Nx+1):
            ...
            if boundary_type[i][0] == NEUMANN:
                ...
                p2 = (phi[i+1][0]+phi[i-1][0])/dx/dx+(phi[i][1] + ghost_phi)/dy/dy
                phi[i][0] = (b[i][0] + mu/rho*p2)/p1
            if boundary_type[i][Ny] == DIRICHLET:
                phi[i][Ny] = boundary_values[i][Ny]
            if boundary_type[i][Ny] == NEUMANN:
                ghost_phi = phi[i][Ny-1]+2.0*dy*boundary_values[i][Ny]
                p1 = 1/dt+2.0*mu/rho*(1.0/dx/dx+1.0/dy/dy)
                p2 = (phi[i+1][Ny]+phi[i-1][Ny])/dx/dx+(ghost_phi + phi[i][Ny-1])/dy/dy
                phi[i][Ny] = (b[i][Ny] + mu/rho*p2)/p1
