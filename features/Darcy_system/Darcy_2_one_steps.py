...
F1 = inner((P-P_n)/dt, Q)*dx + M_b*inner(J*F_1*K*F_T*grad(P),grad(Q*f(J)))*dx-E(M_n, M_n_1, J, J1)*Q*dx
a1 = lhs(F1)
L1 = rhs(F1)
print(a1)
print(L1)

F2 = inner((M-M_n)/dt,N)*dx -rho*S*N*dx + rho*div(W)*N*dx
a2 = lhs(F2)
L2 = rhs(F2)

#Create VTK file for saving solution
vtkfile_p = File('darcy3/apressure.pvd')
vtkfile_w = File('darcy3/velocity.pvd')
vtkfile_m_1 = File('darcy3/addmass_1.pvd')
vtkfile_m_2 = File('darcy3/addmass_2.pvd')
vtkfile_X = File('darcy3/displacement.pvd')
...
print(P(0.5,0.5))
# print(np.sqrt((P(0.5,0.5)-499.99994373241265 )*(P(0.5,0.5)-499.99994373241265)))"""不加流固耦合程序, Richardson论文中Darcy系统的验证且判断其是否收敛""" 
