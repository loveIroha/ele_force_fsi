...
// Downloaded from https://github.com/stochasticHydroTools/IBMethod
...
double flex6pt(double r, double K) {
    ...
    gamma = 0.25
            * (0.5 * (161. / 36 - 59. / 6 * K + 5 * K * K) * R2 + 1. / 3 * (-109. / 24 + 5 * K) * R2 * R2
               + 5. / 18 * R3 * R3);
    ...
    if (-3 < r && r <= -2) {
        ...
        phi = -3. / (2 * alpha) * (-beta + sgn * sqrt(discr)) - 1. / 16 + 1. / 8 * (K + (r + 2) * (r + 2))
              + 1. / 12 * (3 * K - 1) * (r + 2) + 1. / 12 * (r + 2) * (r + 2) * (r + 2);
    } else if (-1 < r && r <= 0) {
        phi = 2. / (2 * alpha) * (-beta + sgn * sqrt(discr)) + 1. / 4 + 1. / 6 * (4 - 3 * K) * (r + 1)
              - 1. / 6 * (r + 1) * (r + 1) * (r + 1);
    } else if (0 < r && r <= 1) {
        phi = 2. / (2 * alpha) * (-beta + sgn * sqrt(discr)) + 5. / 8 - 1. / 4 * (K + r * r);
    } else if (1 < r && r <= 2) {
        phi = -3. / (2 * alpha) * (-beta + sgn * sqrt(discr)) + 1. / 4 - 1. / 6 * (4 - 3 * K) * (r - 1)
              + 1. / 6 * (r - 1) * (r - 1) * (r - 1);
    } else if (2 < r && r <= 3) {
        phi = 1. / (2 * alpha) * (-beta + sgn * sqrt(discr)) - 1. / 16 + 1. / 8 * (K + (r - 2) * (r - 2))
              - 1. / 12 * (3 * K - 1) * (r - 2) - 1. / 12 * (r - 2) * (r - 2) * (r - 2);
    } else {
        phi = 0.;
    }
...
double flex5pt(double x, double K) {
    ...
    phi = (136 - 40 * K - 40 * r2
           + sqrt(2)
                 * sqrt(3123 - 6840 * K + 3600 * K2 - 12440 * r2 + 25680 * K * r2 - 12600 * K2 * r2 + 8080 * r4
                        - 8400 * K * r4 - 1400 * r6))
          / 280.;
...
double flex5pt_d(double x, double K) {
    ...
    disc = sqrt(3123 - 6840 * K + 3600 * K2 - 12440 * r2 + 25680 * K * r2 - 12600 * K2 * r2 + 8080 * r4 - 8400 * K * r4
                - 1400 * r6);
    dphi = (-40 * 2 * r
            + sqrt(2) * (1. / 2. / disc)
                  * (-24880 * r + 25680 * K * 2 * r - 12600 * K2 * 2 * r + 8080 * 4 * r3 - 8400 * K * 4 * r3
                     - 1400 * 6 * r5))
           / 280.;
...
void phiweights(double r[], char* kernel, double K) {
    ...
    if (strcmp(kernel, "bspline4pt") == 0) {
        ...
        r[0] = 0.;
        r[5] = 0.; /* 4-pt kernel, 1st, last are zero */
