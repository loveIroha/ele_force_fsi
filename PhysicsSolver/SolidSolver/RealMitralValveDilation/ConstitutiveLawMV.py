...
# 标号
marker_tube = 5                 # 直管
marker_commsiure = 8            # 前瓣膜叶和后瓣膜叶的缝合区域
marker_disk = 6                 # 瓣膜环
marker_anterior = 7             # 前瓣膜叶
marker_posterior = 8            # 后瓣膜叶
marker_chordae = 10             # 腱索
marker_papillary_anterior = 11  # 前乳头肌
marker_papillary_posterior = 12  # 后乳头肌(更远)
marker_valve_edge = 5099        # 瓣膜边缘
marker_disk_edge = 1000         # 圆盘边缘
...
def penalty_disk(X):
    ...
    # 计算环面扩大半径
    t_local = max_value(t_current, 0.0)
    t_local = min_value(t_current, t_start_closing)
    a = 1.0*t_local/t_start_closing
    # 计算惩罚向量
    r = sqrt((X0[0]-5.0)*(X0[0]-5.0) + (X0[1]-5.0)*(X0[1]-5.0))
    theta = (r + a) / r
    X_new = (5.0 + theta*(X0[0]-5.0) - X0[0],  5.0 + theta*(X0[1]-5.0))
    return beta*as_vector((X[0] - X_new[0], X[1] - X_new[1], 0.0))


def penalty_papillary(X, move_Y=0.0, move_Z=0.0):
    # 当前时间 t_current
    ...
    t_local = min_value(t_current, t_start_closing)     # 限制 t_local <= 0.185 
    # 乳头肌需要通过惩罚项指定位移
    X_penalty = as_vector(
        (0.0, X[1], X[2])) - as_vector((0.0, move_Y, move_Z))*t_local/t_start_closing
...
H += inner(PK1_disk(X),      grad(V)) * \
    dx(subdomain_id=marker_disk,  degree=5)
# 不可压惩罚项
H += inner(PK1_incompressible(X), grad(V))*dx(degree=5)

# 移动圆盘的边界
H += inner(penalty_disk(X), V)*ds(subdomain_id=marker_disk_edge, degree=5)

# 只固定乳头肌
H += inner(penalty_papillary(X), V)*dx(subdomain_id=marker_papillary_anterior, degree=5)
H += inner(penalty_papillary(X), V)*dx(subdomain_id=marker_papillary_posterior, degree=5)
