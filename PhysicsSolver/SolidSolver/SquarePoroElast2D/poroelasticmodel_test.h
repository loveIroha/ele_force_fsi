...
class LeftWall : public SubDomain {
    bool inside(const Array<double>& x, bool on_boundary) const { return on_boundary and near(x[0], 0.0); }
};
class RightWall : public SubDomain {
    bool inside(const Array<double>& x, bool on_boundary) const { return on_boundary and near(x[0], 1.0); }
};
...
class Displacement : public Expression {
  public:
    Displacement() : t(0.0), Expression(2) {}

    void eval(Array<double>& values, const Array<double>& x) const {
        values[0] = 0.1 * (x[0] * x[0]) * (1.0 - exp(-t * t / 0.25));
        values[1] = 0.0;
    }
    double t;
};
...

{
  public:
    ...
    PoroelasticModelSolver(std::shared_ptr<Mesh> mesh, std::shared_ptr<MeshFunction<size_t>> boundaries, double dt)
        ...
        _boundaries = std::make_shared<MeshFunction<size_t>>(mesh, 1, 0);
        LeftWall  left_wall{};
        RightWall right_wall{};
        left_wall.mark(*_boundaries, 1);
        right_wall.mark(*_boundaries, 3);

        _dt = dt;

        // Define the boundary conditions
        pressure_inflow = std::make_shared<InflowPressure>();
        displacement    = std::make_shared<Displacement>();
    ...
    void solve_s(std::vector<double> vector_X
        ...
        std::cout << "size of vector X: " << vector_X.size() << std::endl;
        std::cout << "size of X->vector()->size(): " << X->vector()->size() << std::endl;
        // X->vector()->set_local(vector_X);
        ...
        std::cout << "displacement->t :  " << displacement->t << std::endl;
        std::cout << "pressure_inflow->t :  " << pressure_inflow->t << std::endl;
        X->interpolate(*displacement);
        X->vector()->get_local(vector_X);

        X->eval(values, x);
        std::cout << "before calling solve_s_end, X(0.5,0.5) is " << std::fixed << std::setprecision(15) << values[0]
                  << std::endl;

        solve_s_before(X, X_1, M_1, M_p1, P_1, P, W, M, M2);
        P->eval(values, x);
        std::cout << "before calling solve_s_end, P(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << std::endl;
        W->eval(values, x);
        std::cout << "before calling solve_s_end, W(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << values[1] << std::endl;
        M2->eval(values, x);
        std::cout << "before calling solve_s_end, M(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << std::endl;
        solve_s_end(X, X_1, M_1, M_p1, P_1, P, W, M, M2);
        P->eval(values, x);
        std::cout << "after calling solve_s_end, P(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << std::endl;
        W->eval(values, x);
        std::cout << "after calling solve_s_end, W(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << values[1] << std::endl;
        M->eval(values, x);
        std::cout << "after calling solve_s_end, M(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0]
                  << std::endl;

        std::cout << "sink_source" << std::endl;
        solve_sink_source(W, S);

        S->eval(values, x);
        std::cout << "finally, S(0.5,0.5) is " << std::fixed << std::setprecision(10) << values[0] << std::endl;

        X_1->vector()->set_local(vector_X);
    }

    void get_perfusion_source(std::vector<double>& perfusion_source) { S->vector()->get_local(perfusion_source); }
