...
struct VelocityUKokkos3D {
    ...
    VelocityUKokkos3D(int Nx, int Ny, int Nz, double dt, double Lx, double Ly, double Lz, double mu, double rho,
                      double tolerance, int max_iters)
        : Nx(Nx), Ny(Ny), Nz(Nz), Lx(Lx), Ly(Ly), Lz(Lz), dx(Lx / Nx), dy(Ly / Ny), dz(Lz / Nz), dt(dt), mu(mu),
          rho(rho), tolerance(tolerance), max_iters(max_iters), xh("xh", Nx + 3, Ny + 2, Nz + 2),
          xb("xb", Nx + 3, Ny + 2, Nz + 2), xr("xr", Nx + 3, Ny + 2, Nz + 2),
          x_prime("x_prime", Nx + 3, Ny + 2, Nz + 2), r_prime("r_prime", Nx + 3, Ny + 2, Nz + 2),
          xbcv("xbcv", Nx + 3, Ny + 2, Nz + 2), xbct("xbct", Nx + 3, Ny + 2, Nz + 2) {
    ...
    struct GenerateXPrime {
        ...
        void operator()(int i, int j) const {
            int x, y, z;
            if (Surface == LEFT) { x = 1, y = i + 1, z = j + 1; }
            if (Surface == RIGHT) { x = Nx + 1, y = i + 1, z = j + 1; }
            if (Surface == DOWN) { x = i, y = 0, z = j + 1; }
            if (Surface == UP) { x = i, y = Ny + 1, z = j + 1; }
            if (Surface == FRONT) { x = i, y = j + 1, z = 0; }
            if (Surface == BACK) { x = i, y = j + 1, z = Nz + 1; }

            if (Surface == LEFT) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = xbcv(x, y, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x - 1, y, z) = -2 * dx * xbcv(x, y, z) + x_prime(x + 1, y, z); }
            }
            if (Surface == RIGHT) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = xbcv(x, y, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x + 1, y, z) = 2 * dx * xbcv(x, y, z) + x_prime(x - 1, y, z); }
            ...
            if (Surface == DOWN) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = 2 * xbcv(x, y, z) - x_prime(x, y + 1, z); }
    ...
    void generate_x_prime() {
        ...
        Kokkos::parallel_for("GXP_Left", policy_t({0, 0}, {Ny, Nz}), gxp_left);
        Kokkos::parallel_for("GXP_right", policy_t({0, 0}, {Ny, Nz}), gxp_right);
        Kokkos::parallel_for("GXP_down", policy_t({1, 0}, {Nx + 2, Nz}), gxp_down);
        Kokkos::parallel_for("GXP_up", policy_t({1, 0}, {Nx + 2, Nz}), gxp_up);
        Kokkos::parallel_for("GXP_front", policy_t({1, 0}, {Nx + 2, Ny}), gxp_front);
        Kokkos::parallel_for("GXP_back", policy_t({1, 0}, {Nx + 2, Ny}), gxp_back);
    }
    ...
    struct SmoothBoundary {
        ...
        void operator()(int i, int j) const {
            ...
            if (Surface == LEFT) {
                ...
                if (xbct(x, y, z) == NEUMANN) { x_prime(x - 1, y, z) = x_prime(x + 1, y, z); }
            }
            if (Surface == RIGHT) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = 0.0; }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x + 1, y, z) = x_prime(x - 1, y, z); }
    ...
    void smooth_boundary() {
        ...
        Kokkos::parallel_for("SmoothBoundary_Left", policy_t({0, 0}, {Ny, Nz}), smooth_boundary_left);
        Kokkos::parallel_for("SmoothBoundary_Right", policy_t({0, 0}, {Ny, Nz}), smooth_boundary_right);
        Kokkos::parallel_for("SmoothBoundary_Down", policy_t({1, 0}, {Nx + 2, Nz}), smooth_boundary_down);
        Kokkos::parallel_for("SmoothBoundary_Up", policy_t({1, 0}, {Nx + 2, Nz}), smooth_boundary_up);
        Kokkos::parallel_for("SmoothBoundary_front", policy_t({1, 0}, {Nx + 2, Ny}), smooth_boundary_front);
        Kokkos::parallel_for("SmoothBoundary_back", policy_t({1, 0}, {Nx + 2, Ny}), smooth_boundary_back);
    }
    ...
    struct Residual {
        ...
        void operator()(int i, int j) const {
            for (int k = 0; k < Nz; ++k) {
                if (xbct(i, j + 1, k + 1) == DIRICHLET) continue;
                double b1           = (xh(i - 1, j + 1, k + 1) + xh(i + 1, j + 1, k + 1)) / (dx * dx);
                double b2           = (xh(i, j, k + 1) + xh(i, j + 2, k + 1)) / (dy * dy);
                double b3           = (xh(i, j + 1, k) + xh(i, j + 1, k + 2)) / (dz * dz);
                double b            = mu * (b1 + b2 + b3);
                double a            = rho / dt + 2 * mu * (1 / (dx * dx) + 1 / (dy * dy) + 1 / (dz * dz));
                xr(i, j + 1, k + 1) = xb(i, j + 1, k + 1) + b - a * xh(i, j + 1, k + 1);
            }
        }
    };
    ...
    void generate_r_prime() {
        ...
        Kokkos::parallel_for("Residual", policy_t({1, 0}, {Nx + 2, Ny}), grp);
    }
    ...
    struct SmoothInner {
        ...
        void operator()(int i, int j) const {
            for (int k = 0; k < Nz; k++) {
                if (xbct(i, j + 1, k + 1) == DIRICHLET) continue;
                if ((i + j + k) % 2 == color) {
                    T b1                = (xh(i - 1, j + 1, k + 1) + xh(i + 1, j + 1, k + 1)) / (dx * dx);
                    T b2                = (xh(i, j, k + 1) + xh(i, j + 2, k + 1)) / (dy * dy);
                    T b3                = (xh(i, j + 1, k) + xh(i, j + 1, k + 2)) / (dz * dz);
                    T b                 = mu * (b1 + b2 + b3);
                    T a                 = rho / dt + 2 * mu * (1 / (dx * dx) + 1 / (dy * dy) + 1 / (dz * dz));
                    xh(i, j + 1, k + 1) = (xb(i, j + 1, k + 1) + b) / a;
                }
            }
        }
    };
    ...
    void smooth_inner(View3D<double>& xh, const View3D<int>& bct, const View3D<double>& xb, int3 N, double3 L) {
        ...
        Kokkos::parallel_for("SmoothInner0", policy_t({1, 0}, {N.x + 2, N.y}), smooth_inner_0);
        Kokkos::parallel_for("SmoothInner1", policy_t({1, 0}, {N.x + 2, N.y}), smooth_inner_1);
    }
};
} // namespace mykokkos