...
struct VelocityWKokkos3D {
    ...
    VelocityWKokkos3D(int Nx, int Ny, int Nz, double dt, double Lx, double Ly, double Lz, double mu, double rho,
                      double tolerance, int max_iters)
        : Nx(Nx), Ny(Ny), Nz(Nz), Lx(Lx), Ly(Ly), Lz(Lz), dx(Lx / Nx), dy(Ly / Ny), dz(Lz / Nz), dt(dt), mu(mu),
          rho(rho), tolerance(tolerance), max_iters(max_iters), xh("xh", Nx + 2, Ny + 2, Nz + 3),
          xb("xb", Nx + 2, Ny + 2, Nz + 3), xr("xr", Nx + 2, Ny + 2, Nz + 3),
          x_prime("x_prime", Nx + 2, Ny + 2, Nz + 3), r_prime("r_prime", Nx + 2, Ny + 2, Nz + 3),
          xbcv("xbcv", Nx + 2, Ny + 2, Nz + 3), xbct("xbct", Nx + 2, Ny + 2, Nz + 3) {
    ...
    struct GenerateXPrime {
        ...
        void operator()(int i, int j) const {
            int x, y, z;
            if (Surface == LEFT) { x = 0, y = i + 1, z = j; }
            if (Surface == RIGHT) { x = Nx + 1, y = i + 1, z = j; }
            if (Surface == DOWN) { x = i + 1, y = 0, z = j; }
            if (Surface == UP) { x = i + 1, y = Ny + 1, z = j; }
            if (Surface == FRONT) { x = i + 1, y = j + 1, z = 1; }
            ...
            if (Surface == DOWN) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = 2.0 * xbcv(x, y, z) - x_prime(x, y + 1, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z) = -dy * xbcv(x, y, z) + x_prime(x, y + 1, z); }
            }
            if (Surface == UP) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = 2.0 * xbcv(x, y, z) - x_prime(x, y - 1, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z) = dy * xbcv(x, y, z) + x_prime(x, y - 1, z); }
            ...
            if (Surface == FRONT) {
                ...
                if (xbct(x, y, z) == NEUMANN) {
                    x_prime(x, y, z - 1) = -2.0 * dz * xbcv(x, y, z) + x_prime(x, y, z + 1);
                }
            }
            if (Surface == BACK) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = xbcv(x, y, z); }
    ...
    void generate_x_prime() {
        ...
        Kokkos::parallel_for("GXP_Left", policy_t({0, 1}, {Ny, Nz + 2}), gxp_left);
        Kokkos::parallel_for("GXP_right", policy_t({0, 1}, {Ny, Nz + 2}), gxp_right);
        Kokkos::parallel_for("GXP_down", policy_t({0, 1}, {Nx, Nz + 2}), gxp_down);
        Kokkos::parallel_for("GXP_up", policy_t({0, 1}, {Nx, Nz + 2}), gxp_up);
        Kokkos::parallel_for("GXP_front", policy_t({0, 0}, {Nx, Ny}), gxp_front);
        Kokkos::parallel_for("GXP_back", policy_t({0, 0}, {Nx, Ny}), gxp_back);
    }
    ...
    struct SmoothBoundary {
        ...
        void operator()(int i, int j) const {
            ...
            if (Surface == DOWN) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = -x_prime(x, y + 1, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z) = x_prime(x, y + 1, z); }
            }
            if (Surface == UP) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = -x_prime(x, y - 1, z); }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z) = x_prime(x, y - 1, z); }
            ...
            if (Surface == FRONT) {
                ...
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z - 1) = x_prime(x, y, z + 1); }
            }
            if (Surface == BACK) {
                if (xbct(x, y, z) == DIRICHLET) { x_prime(x, y, z) = 0.0; }
                if (xbct(x, y, z) == NEUMANN) { x_prime(x, y, z + 1) = x_prime(x, y, z - 1); }
    ...
    void smooth_boundary() {
        ...
        Kokkos::parallel_for("SmoothBoundary_Left", policy_t({0, 1}, {Ny, Nz + 2}), smooth_boundary_left);
        Kokkos::parallel_for("SmoothBoundary_Right", policy_t({0, 1}, {Ny, Nz + 2}), smooth_boundary_right);
        Kokkos::parallel_for("SmoothBoundary_Down", policy_t({0, 1}, {Nx, Nz + 2}), smooth_boundary_down);
        Kokkos::parallel_for("SmoothBoundary_Up", policy_t({0, 1}, {Nx, Nz + 2}), smooth_boundary_up);
        Kokkos::parallel_for("SmoothBoundary_front", policy_t({0, 0}, {Nx, Ny}), smooth_boundary_front);
        Kokkos::parallel_for("SmoothBoundary_back", policy_t({0, 0}, {Nx, Ny}), smooth_boundary_back);
    }
    ...
    struct Residual {
        ...
        void operator()(int i, int j) const {
            for (int k = 1; k < Nz + 2; ++k) {
                if (xbct(i + 1, j + 1, k) == DIRICHLET) continue;
                double b1 = (xh(i, j + 1, k) - 2.0 * xh(i + 1, j + 1, k) + xh(i + 2, j + 1, k)) / (dx * dx);
                double b2 = (xh(i + 1, j, k) - 2.0 * xh(i + 1, j + 1, k) + xh(i + 1, j + 2, k)) / (dy * dy);
                double b3 = (xh(i + 1, j + 1, k - 1) - 2.0 * xh(i + 1, j + 1, k) + xh(i + 1, j + 1, k + 1)) / (dz * dz);
                xr(i + 1, j + 1, k) = xb(i + 1, j + 1, k) - rho / dt * xh(i + 1, j + 1, k) + mu * (b1 + b2 + b3);
            }
        }
    };
    ...
    void generate_r_prime() {
        ...
        Kokkos::parallel_for("GenerateRPrime", policy_t({0, 0}, {Nx, Ny}), grp);
    }
    ...
    void residual() {
        ...
        Kokkos::parallel_for("Residual", policy_t({0, 0}, {Nx, Ny}), grp);
    }
    ...
    struct SmoothInner {
        ...
        void operator()(int i, int j) const {
            for (int k = 1; k < Nz + 2; k++) {
                if (xbct(i + 1, j + 1, k) == DIRICHLET) continue;
                if ((i + j + k) % 2 == color) {
                    T b1                = (xh(i, j + 1, k) + xh(i + 2, j + 1, k)) / (dx * dx);
                    T b2                = (xh(i + 1, j, k) + xh(i + 1, j + 2, k)) / (dy * dy);
                    T b3                = (xh(i + 1, j + 1, k - 1) + xh(i + 1, j + 1, k + 1)) / (dz * dz);
                    T b                 = mu * (b1 + b2 + b3);
                    T a                 = rho / dt + 2 * mu * (1 / (dx * dx) + 1 / (dy * dy) + 1 / (dz * dz));
                    xh(i + 1, j + 1, k) = (xb(i + 1, j + 1, k) + b) / a;
                }
            }
        }
    };
    ...
    void smooth_inner(View3D<double>& xh, const View3D<int>& bct, const View3D<double>& xb, int3 N, double3 L) {
        ...
        Kokkos::parallel_for("SmoothInner0", policy_t({0, 0}, {N.x, N.y}), smooth_inner_0);
        Kokkos::parallel_for("SmoothInner1", policy_t({0, 0}, {N.x, N.y}), smooth_inner_1);
    }
};

// void inline solve_w(VelocityWKokkos3D& kokkos_solver, Array3D<double>& rh,
// Array3D<double>& xh,
