...
struct CudaSurfaceAccessor
{
    ...
    template <cudaSurfaceBoundaryMode mode = cudaBoundaryModeTrap>
    __device__ __forceinline__ double read_double(int x, int y, int z) const
    {
        if constexpr (sizeof(T) != sizeof(double))
        {
            static_assert(sizeof(T) == sizeof(double), "sizeof(T) != sizeof(double)");
        }
        T temp_0 = surf3Dread<T>(m_cuSuf, x * sizeof(T), y, z, mode);
        double *temp_1 = (double *)(&temp_0);
        return *temp_1;
    }

    template <cudaSurfaceBoundaryMode mode = cudaBoundaryModeTrap>
    __device__ __forceinline__ void write_double(double val, int x, int y, int z) const
    {
        if constexpr (sizeof(T) != sizeof(double))
        {
            static_assert(sizeof(T) == sizeof(double), "sizeof(T) != sizeof(double)");
        ...
        T *temp_0 = (T *)(&val);
        surf3Dwrite<T>(*temp_0, m_cuSuf, x * sizeof(T), y, z, mode);
    }
};

// struct CudaSurfaceAccessorDouble
// {
//     cudaSurfaceObject_t m_cuSuf;

//     template <cudaSurfaceBoundaryMode mode = cudaBoundaryModeTrap>
//     __device__ __forceinline__ double read(int x, int y, int z) const
//     {
//         float2 temp_0 = surf3Dread<float2>(m_cuSuf, x * sizeof(float2), y, z, mode);
//         double *temp_1 = (double *)temp_0;
//         return *temp_1;
//     }

//     template <cudaSurfaceBoundaryMode mode = cudaBoundaryModeTrap>
//     __device__ __forceinline__ void write(double val, int x, int y, int z) const
//     {
//         double* temp_0 = (float2*)val
//         surf3Dwrite<float2>(*temp_0, m_cuSuf, x * sizeof(float2), y, z, mode);
//     }
// };
...
using CudaSurfaceDouble = CudaSurface<float2>;
