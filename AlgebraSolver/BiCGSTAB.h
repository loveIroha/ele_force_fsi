...
//   title={Comparison of Some Preconditioners for the Incompressible
//   Navier-Stokes Equations}, author={ He, X.  and  Vuik, C. },
...
//        (t_j, t_j) = 0, (t_j, s_j) = 0, \sqrt{(t_{j+1}, t_{j+1})} = 0, \beta =
//        0
...
// #ifdef PENGFEI_CXX_STANDARD
// // static_assert(PENGFEI_CXX_STANDARD == 17, "PENGFEI_CXX_STANDARD must
// be 17."); #if (PENGFEI_CXX_STANDARD == 20) #include<concepts>
// template<typename VectorType>
// concept duck_type = requires(VectorType vector) {
...
class BiCGSTAB : public LinearSolver<VectorType> {
  ...
  public:
    ...
    // $\frac{1}{\kappa(A)} \frac{\|\mathbf{r}\|}{\|\mathbf{b}\|} \leq
    // \frac{\|\mathbf{e}\|}{\|\mathbf{u}\|} \leq \kappa(A)
    // \frac{\|\mathbf{r}\|}{\|\mathbf{b}\|}$ Which means it can be controled by $
    // \frac{\|r\|}{\|b\|} $.
    //
    ...
    virtual std::pair<bool, std::pair<double, int>> Solve(std::shared_ptr<LinearProblem<VectorType>> linear_problem,
        ...
        *p = *r0;                       // p = r0
        ...
        if (norm_b / b->size() < 1e-12) {
            *x0 = 0.0;
            return std::make_pair(true, std::make_pair(0.0, iter));
        }
        double relative_error       = std::sqrt(r->inner(*r)) / norm_b;
        double relative_error_start = relative_error;

        LOG_F(INFO, "norm_b = %e, relative_error = %e, size = %zu", norm_b, relative_error, x0->size());

        double delta, gamma;

        // start the iterations
        while (iter < this->get_max_iteration()) {
            LOG_F(WARNING, "Inside the BICGSTAB iteration, iter = %d, relative_error = %e", iter, relative_error);
            iter++;
            if (!this->silent) LOG_F(INFO, "BiCGSTAB solver(start) : iteration %d.", iter);
            ...
            if (!std::isnormal(omega)) {
                LOG_F(WARNING, "At the end of BiCGSTAB solver, the relative_error norm is %e.", relative_error);
                return std::make_pair(true, std::make_pair(relative_error, iter));
            }
            // x = x + alpha*p + omega*s
            ...
            *temp = *r;                 // r_j
            ...
            if (!this->silent) LOG_F(WARNING, "BiCGSTAB solver(end) : iteration %d.", iter);

            if (relative_error < this->get_tolerance() * relative_error_start) {
                LOG_F(WARNING, "At the end of BiCGSTAB solver, the relative_error norm is %e.", relative_error);
                return std::make_pair(true, std::make_pair(relative_error, iter + 1));
            }
    ...
    BiCGSTAB(size_t n) { Initialize(n); }
