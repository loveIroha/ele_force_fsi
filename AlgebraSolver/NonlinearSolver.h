...
//
// Despite the perception that it is fragile or scary, Newton’s method works
// well on many nonlinear systems if one has a good initial iterate x0 and one
// adds some important protections. A line search, which sometimes moves a
// shorter distance than computed, is such a “protection”.
//                                                              -- PETSc for
//                                                              Partial
//                                                              Differenctial
//                                                              Equations
...
class NonlinearSolver {
  ...
  public:
    ...
    void set_max_iteration(int iter) { max_nonlinear_iteration = iter; }

    int get_max_iteration() const { return max_nonlinear_iteration; }

    void set_tolerance(double tol) { max_nonlinear_tolerance = tol; }

    double get_tolerance() const { return max_nonlinear_tolerance; }
  ...
  public:
    ...
    Solve(std::shared_ptr<NonlinearProblem<VectorType>> nonlinear_problem, std::shared_ptr<VectorType> x0,
          std::shared_ptr<const VectorType> b)
        = 0;
