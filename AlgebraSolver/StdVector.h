...
#include <AlgebraSolver/GenericVector.h>
...
class StdVector : public GenericVector<T, TV> {
  ...
  public:
    ...
    virtual double inner(const GenericVector<T, TV>& x) const override {
        ...
        T sum = 0.0;
        for (size_t i = 0; i < _x.num; i++)
            sum += _x.data[i] * _m.data[i];
        return sum;
    }
    ...
    virtual void axpy(T a, const GenericVector<T, TV>& y) override {
        ...
        for (size_t i = 0; i < _x.num; i++)
            _x.data[i] = _y.data[i] + a * _x.data[i];
    }
    ...
    virtual void axpy(T a, const GenericVector<T, TV>& x, const GenericVector<T, TV>& y) override {
        ...
        for (size_t i = 0; i < _x.num; i++)
            _z.data[i] = a * _x.data[i] + _y.data[i];
    }
    ...
    explicit StdVector(std::vector<TV> data) : StdVector() {
    ...
    explicit StdVector(int dimx, int dimy = 1, int dimz = 1) : StdVector(int3{dimx, dimy, dimz}) {}
