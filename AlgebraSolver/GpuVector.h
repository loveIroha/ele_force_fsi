...
// https://blog.csdn.net/zerolover/article/details/44458985
...
class GpuVector : public GenericVector<T, TV> {
  ...
  public:
    ...
    virtual const GenericVector<T, TV>& operator-= (T a) override {
        CHECK_F(false, "not implemented. (The parameter is %f.)", a);
    }
    ...
    // Five functions
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // TODO：拷贝赋值运算符，要求任意GenericVector之间可以相互赋值。
    // 1. 参考Dolfin的代码
    // 2. 参考https://www.cnblogs.com/zpcdbky/p/5027481.html
    // virtual const GpuVector& operator= (const GpuVector& x) final
    virtual GpuVector& operator=(const GpuVector& x) {
        if (this == &x) return *this; // 避免自赋值
        CHECK_F(this->use_gpu() == x.use_gpu(), "not on GPU.");
        CHECK_F(x.size() == this->size(), "Wrong size.");
        CHECK_F(x.dim().x == this->dim().x && x.dim().y == this->dim().y && x.dim().z == this->dim().z, "Wrong dim.");
        // LOG_F(WARNING, "gpu_copy on assignment method.");  // 性能监视
        gpu::gpu_copy((char*)this->data(), (char*)x.data(), this->size() * sizeof(TV));
        return *this;
    }

    // 从cpu_vector复制到gpu_vector
    // NOTE: 为什么要使用引用返回值？
    const GpuVector& operator=(const StdVector<T, TV>& cpu_vector) {
        CHECK_F(cpu_vector.size() == this->size(), "Wrong size.");
        gpu::cpu_to_gpu((char*)_data, (char*)cpu_vector.data(), sizeof(TV) * this->size());
        return *this;
    }
    ...
    explicit GpuVector(GpuVector&& gpu_vector) : GpuVector() {
        // there is no need to care about freeing memory, because it is a
        // constructor.
        this->_data = gpu_vector.data();
        this->_dim  = gpu_vector.dim();
        gpu_vector.set_nullptr();
