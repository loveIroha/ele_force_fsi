...
// -----------------------------------------------------------------------------------
// GenericVector<T, TV> is an interface class, but I don't know how to complete
// it now. blogs for future study : C++：如何正确的使用接口类
...
inline Array1D<double> flatten(const std::vector<double3>& x) {
    Array1D<double> res;
    res.data = (double*)x.data();
    res.num  = 3 * x.size();
    return res;
}
...
class GenericVector {
  ...
  public:
    ...
    // NOTE : Try not to use these operator, because it is expensive for data
    // stored in GPU memory. virtual double& operator[](std::size_t __n) = 0;
    ...
    /// Assignment operator
    // TODO : what's the difference between GenericVector& and const
    // GenericVector& as return type? StdVector<double, double3>&
    // StdVector<double, double3>::operator=(const StdVector<double, double3>&) do
    // not override this function. virtual GenericVector& operator= (const
    // GenericVector& x) = default;
