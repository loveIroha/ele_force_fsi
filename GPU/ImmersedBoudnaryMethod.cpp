...
double ibm_phi(double r) {
    double res = 0;
    double r2  = r * r;
    if (r <= 2) res = 0.125 * (5 - 2 * r - sqrt(-7 + 12 * r - 4 * r2));
    if (r <= 1) res = 0.125 * (3 - 2 * r + sqrt(1 + 4 * r - 4 * r2));
    return res;
}
        ...
        for (int kk = k - 1; kk <= k + 2; kk++) {
            for (int jj = j - 1; jj <= j + 2; jj++) {
                for (int ii = i - 1; ii <= i + 2; ii++) {
                    if (!(kk >= 0 && kk < dim.z && jj >= 0 && jj < dim.y && ii >= 0 && ii < dim.x)) { continue; }
                    int     widx = ii + jj * dim.x + kk * dim.x * dim.y;
                    double3 spreadf;
                    double  weight = w * inv_h3
                                    * ibm_delta3(make_double3(quadrature_rule.x / h, quadrature_rule.y / h,
                                                              quadrature_rule.z / h),
        ...
        for (int kk = k - 1; kk <= k + 2; kk++) {
            for (int jj = j - 1; jj <= j + 2; jj++) {
                for (int ii = i - 1; ii <= i + 2; ii++) {
                    ...
                    int    ridx   = ii + jj * dim.x + kk * dim.x * dim.y;
                    double weight = ibm_delta3(make_double3(pos.x / h, pos.y / h, pos.z / h), make_double3(ii, jj, kk));
                    ...
                    // printf("interpolate_velocity_kernel : %d, %.12e, %.12e,
